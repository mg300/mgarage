...
function Page() {
  ...
  const handleCheck = function (id: string) {
    ...
    if (IDs.includes(id)) {
      ...
      IDs.push(id);
      const index = data.findIndex((service) => service.id === Number(id));
    ...
    <div className="min-h-screen pt-40 font-body  mx-auto max-w-[80rem] ">
      ...
      <div className=" relative p-16 border-2 border-solid border-gray-200 shadow-lg min-h-[40rem] mb-20">
        <div className="flex flex-col lg:flex-row gap-36 mb-20">
          ...
          <form>
            <ul className="flex flex-col gap-5">
              {data.map((service) => (
                <li key={service.id}>
                  <input
                    checked={IDs.includes(service.id.toString())}
                    type="checkbox"
                    onChange={() => handleCheck(service.id.toString())}
                  ...
                  >
                    <p className="text-sm lg:text-lg">{service.name}</p>
          ...
          {indexOfDescr !== null && indexOfDescr >= 0 && (
            <div className="sticky top-10">
              <p className="text-logo font-semibold text-2xl mb-10">{data[indexOfDescr]?.name}</p>
              <div className="bg-gray-200 rounded-3xl pt-2 pb-2.5">
                <div className="leading-snug md:leading-normal p-4">{data[indexOfDescr]?.description}</div>
              </div>
              {data[indexOfDescr].bulletDescription && (
                <>
                  {" "}
                  <p className="mt-10 mb-3 font-semibold text-lg leading-relaxed">Proces usługi:</p>
                  <ul className="list-disc p-4">
                    {data[indexOfDescr].bulletDescription.split("\n").map((point, index) => (
